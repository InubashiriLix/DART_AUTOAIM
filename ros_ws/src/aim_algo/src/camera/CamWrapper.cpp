...
void GX_STDC OnFrameCallbackFun(GX_FRAME_CALLBACK_PARAM *pFrame) {
    if (pFrame->status == GX_FRAME_STATUS_SUCCESS) {
        ...
        ProcessData((void *)pFrame->pImgBuf, cam->g_pRaw8Buffer, cam->g_pRGBframeData,
                    pFrame->nWidth, pFrame->nHeight, pFrame->nPixelFormat, cam->g_nColorFilter);
        ...
        if (cam->is_energy) {
            ...
            cam->pimg_lock.lock();
            cv::resize(cam->p_energy, cam->p_img, cv::Size(ROI_width, ROI_height),
                       cv::INTER_NEAREST);
...
void getRGBImage(DHCamera *cam) {
    while (1) {
        ...
        ProcessData(cam->g_frameBuffer->pImgBuf, cam->g_pRaw8Buffer, cam->g_pRGBframeData,
        ...
        if (cam->is_energy) {
            ...
            memcpy(cam->p_energy.data, cam->g_pRGBframeData, 3 * (cam->nPayLoadSize));
            cam->pimg_lock.lock();
            cv::resize(cam->p_energy, cam->p_img, cv::Size(640, 512), cv::INTER_NEAREST);
            cv::cvtColor(cam->p_img, cam->p_img, cv::COLOR_RGB2BGR);
            cam->pimg_lock.unlock();
        } else {
            cam->pimg_lock.lock();
...
DHCamera::DHCamera(std::string sn)
    : sn(sn),
      ...
      init_success(false),
      is_energy(false) {
    p_img = cv::Mat(ROI_height, ROI_width, CV_8UC3);
    //    p_energy = cv::Mat(1024, 1024, CV_8UC3);
    p_energy = cv::Mat(ROI_height, ROI_width, CV_8UC3);
};
...
bool DHCamera::init(int roi_x, int roi_y, int roi_w, int roi_h, float exposure, float gain,
                    bool isEnergy, float FPS, int nBinning) {
    GXInitLib();
    ...
    if (nDeviceNum >= 1) {
        ...
        // 设置Binning模式
        UPDB(GXSetEnum(g_hDevice, GX_ENUM_BINNING_HORIZONTAL_MODE,
                       GX_BINNING_HORIZONTAL_MODE_AVERAGE),
             "Binning_h_mode");
        UPDB(GXSetEnum(g_hDevice, GX_ENUM_BINNING_VERTICAL_MODE, GX_BINNING_VERTICAL_MODE_AVERAGE),
             "Binning_v_mode");
        ...
        //     "BlacklevelAuto");
        UPDB(GXSetEnum(g_hDevice, GX_ENUM_BALANCE_WHITE_AUTO, GX_BALANCE_WHITE_AUTO_CONTINUOUS),
             "BalanceWhiteAuto");
        ...
        UPDB(GXSetEnum(g_hDevice, GX_ENUM_ACQUISITION_MODE, GX_ACQ_MODE_CONTINUOUS),
             "AcquisitionMode");
        ...
        UPDB(GXSetEnum(g_hDevice, GX_ENUM_ACQUISITION_FRAME_RATE_MODE,
                       GX_ACQUISITION_FRAME_RATE_MODE_ON),
             "FPS_Ctrl");
        UPDB(GXSetFloat(g_hDevice, GX_FLOAT_ACQUISITION_FRAME_RATE, FPS), "FPS");

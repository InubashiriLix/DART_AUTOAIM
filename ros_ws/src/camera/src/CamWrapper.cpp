    ...
    while (1) {
        ...
        if (cam->is_energy) {
            ...
            memcpy(cam->p_energy.data, cam->g_pRGBframeData,
                   3 * (cam->nPayLoadSize));
            cam->pimg_lock.lock();
            cv::resize(cam->p_energy, cam->p_img, cv::Size(640, 512),
                       cv::INTER_NEAREST);
            cv::cvtColor(cam->p_img, cam->p_img, cv::COLOR_RGB2BGR);
            cam->pimg_lock.unlock();
        } else {
...
bool DHCamera::init(int roi_x, int roi_y, int roi_w, int roi_h, float exposure,
    ...
    if (nDeviceNum >= 1) {
        ...
        //设置Binning模式
        UPDB(GXSetEnum(g_hDevice,GX_ENUM_BINNING_HORIZONTAL_MODE,GX_BINNING_HORIZONTAL_MODE_AVERAGE), "Binning_h_mode");
        UPDB(GXSetEnum(g_hDevice,GX_ENUM_BINNING_VERTICAL_MODE,GX_BINNING_VERTICAL_MODE_AVERAGE), "Binning_v_mode");
	...
	UPDB(GXSetEnum(g_hDevice, GX_ENUM_ACQUISITION_FRAME_RATE_MODE, GX_ACQUISITION_FRAME_RATE_MODE_ON), "FPS_Ctrl");

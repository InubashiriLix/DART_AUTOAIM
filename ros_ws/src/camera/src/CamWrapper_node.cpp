...
Mat img_src;
std::string get_flag_option(const std::vector<std::string> &args,
  ...
  CameraPublisher(int argc, char **argv) : Node("camera_publisher") {
    if (FOR_PC) {
      ...
      camera->init((sensor_width / nBinning - ROI_width) / 2,
                   (sensor_height / nBinning - ROI_height) / 2, ROI_width,
                   ROI_height, 1500, 16, false, FPS, nBinning);
      if (!camera->start()) {
      ...
      _image_timer_ = this->create_wall_timer(
          0.001s, std::bind(&CameraPublisher::publish_camera, this));
  ...
  ~CameraPublisher() {
    if (camera != nullptr) {
      RCLCPP_INFO(this->get_logger(),
                  "closing camera instance on quiting node");
      camera->stop();
    }
  }
...
private:
  ...
  void publish_camera_info() {
    ...
    cv::Mat cameraMatrix =
        (cv::Mat_<double>(3, 3) << 1557.2 / nBinning, 0.2065,
         (638.7311 / nBinning) /
             ((((float)sensor_width) / nBinning) / ROI_width),
         0, 1557.5 / nBinning,
         (515.1176 / nBinning) /
             ((((float)sensor_height) / nBinning) / ROI_height),
         0, 0, 1);
    cv::Mat distCoeffs =
        (cv::Mat_<double>(1, 5) << -0.1295, 0.0804, 4.85E-04, 6.37E-04, 0.2375);
    cv::Mat dst = cv::getOptimalNewCameraMatrix(
        cameraMatrix, distCoeffs, cv::Size(ROI_width, ROI_height), 0);
  ...
  void publish_camera() {
    ...
    _cv_bridge = cv_bridge::CvImage(_header, sensor_msgs::image_encodings::BGR8,
                                    img_src);
